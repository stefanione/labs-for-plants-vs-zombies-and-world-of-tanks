...
void Lab4::Init()
{
    ...
    // Initialize tx, ty and tz (the translation steps) // primul cub
    translateX = 0;
    translateY = 0;
    translateZ = 0;

    // Initialize sx, sy and sz (the scale factors) // al doilea cub
    scaleX = 1;
    scaleY = 1;
    scaleZ = 1;

    // Initialize angular steps // al treilea cub
    angularStepOX = 0;
    angularStepOY = 0;
    angularStepOZ = 0;
...
void Lab4::RenderScene() {
    ...
    modelMatrix *= transform3D::Scale(1, 0.35f, 2);
    RenderMesh(meshes["box"], shaders["Green"], modelMatrix);


    //turret
    modelMatrix = glm::mat4(1);
    modelMatrix *= transform3D::Translate(0.0f, 0.75f, -1.5f);
    modelMatrix *= transform3D::RotateOY(angularStepOY);
    modelMatrix *= transform3D::Scale(0.75f, 0.3f, 0.75f);
    RenderMesh(meshes["box"], shaders["Color"], modelMatrix);

    modelMatrix = glm::mat4(1);
    //modelMatrix *= transform3D::Translate(2.5f * cos(increment), 0.5f, (-1) * (2.5f) * sin(increment) - 1.5f);
    modelMatrix *= transform3D::Translate(2.5f, 0.5f, -1.5f);
...
void Lab4::Update(float deltaTimeSeconds)
{
    ...
    modelMatrix = glm::mat4(1);
    //modelMatrix *= transform3D::Translate(2.0f, 1, -1.5f);
    modelMatrix *= transform3D::Translate(2.5f * cos(increment), 0.5f, (-1) * (2.5f) * sin(increment) - 1.5f);
    //modelMatrix *= transform3D::Translate(2.5f, 0.5f, -1.5f);
    modelMatrix *= transform3D::RotateOX(angularStepOX);
    modelMatrix *= transform3D::RotateOY(angularStepOY);
...
void Lab4::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_I) {
        ...
        miniViewportArea.height += (0.1f) * resolution.y;
        miniViewportArea.width += (0.1f) * resolution.x;
    }
