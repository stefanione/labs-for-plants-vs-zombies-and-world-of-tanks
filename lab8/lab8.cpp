...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
   ...
   /* int light_direction2 = glGetUniformLocation(shader->program, "light_direction2");
    ...
    /*glm::vec3 eyePosition2 = GetSceneCamera()->m_transform->GetWorldPosition();

    int eye_position2 = glGetUniformLocation(shader->program, "eye_position2");
    glUniform3f(eye_position2, eyePosition2.x, eyePosition2.y, eyePosition2.z);*/
    ...
    glUniform1i(material_shininess2, materialShininess);
    ...
    GLint isSpotl = glGetUniformLocation(shader->program, "isSpotligth");
    glUniform1i(isSpotl, isSpotligth);

    GLfloat cut_off_angle = glGetUniformLocation(shader->program, "cut_off_angle");
    glUniform1f(cut_off_angle, this->cut_off_angle);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        glm::vec4 ox = glm::rotate(glm::mat4(1.0f), angle * deltaTime, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 1);
        glm::vec4 ox_2 = glm::rotate(glm::mat4(1.0f), (-1) * angle * deltaTime, glm::vec3(1, 0, 0)) * glm::vec4(lightDirection, 1);
        glm::vec4 oz = glm::rotate(glm::mat4(1.0f), angle * deltaTime, glm::vec3(0, 0, 1)) * glm::vec4(lightDirection, 1);
        glm::vec4 oz_2 = glm::rotate(glm::mat4(1.0f), (-1) * angle * deltaTime, glm::vec3(0, 0, 1)) * glm::vec4(lightDirection, 1);

        if (window->KeyHold(GLFW_KEY_1)) lightDirection = glm::vec3(ox);
        if (window->KeyHold(GLFW_KEY_2)) lightDirection = glm::vec3(ox_2);
        if (window->KeyHold(GLFW_KEY_3)) lightDirection = glm::vec3(oz);
        if (window->KeyHold(GLFW_KEY_4)) lightDirection = glm::vec3(oz_2);


        if (window->KeyHold(GLFW_KEY_5)) cut_off_angle += 10 * deltaTime * speed;
        ////cout << "cut : " << cut_off_angle << " \n";
        if (window->KeyHold(GLFW_KEY_6)) cut_off_angle -= 10 * deltaTime * speed;
...
void Lab8::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_F) {
        isSpotligth = 1 - isSpotligth;
        cout << isSpotligth << endl;
    }
